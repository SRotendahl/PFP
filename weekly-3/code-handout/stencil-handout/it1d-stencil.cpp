        ...
        #pragma omp parallel default(shared)
        {
            ...
            for(int32_t tx=0; tx<DIM_X; tx+=T) {
                ...
                for(int32_t q0 = 0; q0 < UB; q0++) {
                    ...
                    for(int32_t x0 = MAX(0,tx-q); x0 < MIN(tx+T+q, DIM_X); x0++) {
		      int32_t ind = x0 - tx;
		      NUM neu = tile[q0%2][ind+FUSEDEG];
		      NUM neg = tile[q0%2][ind+FUSEDEG-1];
		      NUM pos = tile[q0%2][ind+FUSEDEG+1];
		      tile[(q0+1)%2][ind+FUSEDEG] = (neu+neg+pos)/3;
		    }
