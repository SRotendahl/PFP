      ...
      for(int32_t tx=0; tx<DIM_X; tx+=T) {
          ...
          for(int32_t y=-1; y<T+1; y++) {
            for(int32_t x=0; x<T; x++) {
                ...
                //NUM dummy = 3;
	      NUM inm1B, inp1B, incB;
	      incB  = IND(in,modY(ty+y),modX(tx+x));
	      inm1B = IND(in,modY(ty+y),modX(tx+x-1));
	      inp1B = IND(in,modY(ty+y),modX(tx+x+1));
	      blurx[y+1][x] = (incB+inm1B+inp1B)/3;
            }
          }
          ...
          for(int32_t y=1; y<T+1; y++) {
            for(int32_t x=0; x<T; x++) {
                ...
                //NUM dummy = 3;
		NUM inm1O, inp1O, incO;
		incO  = blurx[y][x];
		inm1O = blurx[y-1][x];
		inp1O = blurx[y+1][x];
		IND(out, ind_y, ind_x) = (incO+inm1O+inp1O)/3;
            }
          }
          // end
      }
    ...
    for(int32_t ty=0; ty<DIM_Y; ty+=T) {
        for(int32_t y=-2; y<T; y++) {
            ...
            for(int32_t x=0; x<DIM_X; x++) {
                ...
                //NUM dummy = 3;
	      NUM inm1B, inp1B, incB;
	      incB  = IND(in,modY(ty+y+1),modX(x));
	      inm1B = IND(in,modY(ty+y+1),modX(x-1));
	      inp1B = IND(in,modY(ty+y+1),modX(x+1));
	      blurx[(y+2)%3][x] = (incB+inm1B+inp1B)/3;
            }
            if(y < 0) continue;
		...
		inm1O = blurx[(y+1)%3][x];
		inp1O = blurx[(y+2)%3][x];
                IND(out, ty+y, x) = (incO+inm1O+inp1O)/3;
            }
        }  // end
    }
  }
